...
// Adds currency symbol to label

[HtmlTargetElement("inputCurrencyBox", Attributes = ValueAttributeName)]
public class InputCurrencyTagHelper : TagHelper
{
    ...
    public InputCurrencyTagHelper(IHtmlGenerator generator)
    {
        Generator = generator;
    }
    ...
    public override void Process(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var labelContent = $"<label for='{name}' class='col-sm-3 col-form-label text-end'>{patternContent}{label} {CultureInfo.CurrentCulture.NumberFormat.CurrencySymbol}</label>";
